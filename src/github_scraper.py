...
def _get_pr_reviews(pr_data_endpoint_url:str, pr_number:int, request_headers:dict):
    ...
    response = get(pr_reviews_endpoint_url, headers=request_headers, params=query_params)
...
def get_repo_pull_requests_filterd_data(repo_url:str, days_back:int):
    ...
    filtered_pr_events = []
    while True:
        response = get(pr_data_endpoint_url , headers=request_headers, params=query_params)
        ...
        if len(raw_pr_events) == 0:
            ...
            for raw_pr_event in raw_pr_events:
                ...
                if merged_at is not None and merged_at < earliest_pr_merge_date:
                    ...
                    pr_raw_reviews = _get_pr_reviews(pr_data_endpoint_url, filtered_pr_event['number'], request_headers)
